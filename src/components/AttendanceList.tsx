        ...
        <div className="w-full max-w-2xl mx-auto flex flex-col h-full">
            ...
            <div className="flex items-center justify-between mb-4 sticky top-0 bg-gray-50 py-2 z-10">
                <span className="text-sm font-semibold tracking-wide text-gray-500 uppercase">
                    {totalPresent} / {participants.length} Present
                </span>
                <div className="space-x-2">
                    <button
                        onClick={() => onMarkAll(true)}
                        className="px-3 py-1.5 text-sm font-medium text-blue-700 bg-blue-100 rounded-lg active:bg-blue-200"
                    >
                        Mark All
                    </button>
                    <button
                        onClick={() => onMarkAll(false)}
                        className="px-3 py-1.5 text-sm font-medium text-gray-600 bg-gray-200 rounded-lg active:bg-gray-300"
                    >
                        Clear All
                    </button>
                </div>
            ...
            <div className="flex-1 overflow-y-auto pb-24">
                {participants.length === 0 ? (
                    <div className="text-center text-gray-500 py-10">No names left in this list.</div>
                ) : (
                    participants.map(p => (
                        <NameReviewItem
                            key={p.id}
